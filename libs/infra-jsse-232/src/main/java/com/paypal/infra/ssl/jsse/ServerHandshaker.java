...
import com.paypal.infra.ssl.jsse.HandshakeMessage.*;
import com.paypal.infra.ssl.PayPalJSSEProperties;
import com.paypal.infra.ssl.jsse.CipherSuite.*;
import com.paypal.infra.ssl.jsse.SignatureAndHashAlgorithm.*;
...
final class ServerHandshaker extends Handshaker {
    ...
    ServerHandshaker(SSLSocketImpl socket, SSLContextImpl context,
		...
		// PayPal
		// change to enforce client Auth flag. Read it from protected CFG.
		// this is the bottom guts in the code to override client auth. this
		// setting really enforce auth type
            ...
            if (previous != null) {
                ...
                if (resumingSession &&
                    ...
                    try {
						...
						// PayPal specific change - if PPSessions then peer cert
						// will not be present in sessions hence ignore. check
						// PP cache session flag before
						// setting resume flag to false
						if (!previous.isPPCachedSession())
							resumingSession = false;
						// End
                    }
                }
    ...
    private void clientFinished(Finished mesg) throws IOException {
        ...
        if (doClientAuth == SSLEngineImpl.clauth_required) {
			...
			// PayPal specific change - if PPSessions then peer cert will not be
			// present in sessions hence ignore. check PP cache session flag
			// before
			// setting resume flag to false
			// session.getPeerPrincipal();
			if (!session.isPPCachedSession())
				session.getPeerPrincipal();
			// End
        }
