...
import static com.paypal.infra.ssl.jsse.CipherSuite.KeyExchange.*;
 ...
 */
final class SSLSessionImpl extends ExtendedSSLSession implements PayPalSSLSession{
	...
	/*
	 * PayPal specific changes. set ppSaveSession to true for sessions which are
	 * loaded from PayPal session store. This variable will be used to avoid
	 * peer certificate verification in case of Client Authentication because
	 * Client authentication has already performed while generating session.
	 * This is required mainly because Peer certs are not loaded from saved
	 * session into SSLSessionImpl object. Verified peer cert can be loaded from
	 * saved session, but it will be a costly operation (20 milli sec) and
	 * inherently JSSE DER encoder doesn't support reading of X.509 data. Only
	 * once flag is enough to check session resumed from external cache.
	 * Handshaker will create a new SSLSession if not able to resume session
	 * with this saved session.
	 */
	private boolean ppSavedSession = false;
	...
	// PayPal
	public void setPPSavedSession(boolean b) {
		ppSavedSession = b;
	}

	@Override
	public boolean isPPCachedSession() {
		return ppSavedSession;
	}
