...
package com.paypal.infra.ssl.jsse;
...
import com.paypal.infra.ssl.sessioncacheprovider.SSLSessionCacheProviderFactory;
import com.paypal.infra.ssl.sessioncacheprovider.SessionCacheParameters;
...
final class SSLSessionContextImpl implements SSLSessionContext {
    ...
    SSLSessionContextImpl() {
		...
		/*
		 * PayPal specific change - creating null cache instead of soft memory
		 * cache out this section to avoid storing session into memory cache.
		 * This is not required as sessions always fetch from external cache
		 * (session cache in protected) and PayPal-JSSE doesn't want to store
		 * sessions created on fly.
		 */
        sessionCache = Cache.newNullCache();
        sessionHostPortCache = Cache.newNullCache();
    }

    /**
     * Returns the <code>SSLSession</code> from protected/sessions.cdb via cache provider bound to the specified session id.
     */
    @Override
    public SSLSession getSession(byte[] sessionId) {
		...
		// PayPal
		SessionCacheParameters sessionCacheParameters = SSLSessionCacheProviderFactory.getSSLCacheProvider()
				.getSession(sessionId);

		if (sessionCacheParameters != null) {
			// System.out.println("SSLSessionContextImpl.getSession() -got a
			// session from cache provider");
			SSLSessionImpl sess = null;
			try {
				sess = convertParametersToSSLSession(sessionCacheParameters, null, -1);
			} catch (Exception e) {
				// dont break handshake in any case.
			}
    ...
    /*
     * Only for unit tests. getSession() should be used otherwise.
     */
    protected SSLSession getSessionFromCache(byte[] sessionId) {
    	return sessionCache.get(new SessionId(sessionId));
    }
    ...
    SSLSessionImpl get(String hostname, int port) {
		...
		if (sessionCacheParameters != null) {
			SSLSessionImpl sess = null;
			try {
				sess = convertParametersToSSLSession(sessionCacheParameters, hostname, port);
			} catch (Exception e) {
				// dont break handshake in any case.
			}
			return sess;
	...
	// PayPal
	private SSLSessionImpl convertParametersToSSLSession(SessionCacheParameters session, String hostname, int port) {
		if (session != null) {
			// ProtocolVersion pv = ProtocolVersion.TLS10;
			int version = session.getProtocolVersion();
			int major = (version >> 8) & 0xff;
			int minor = version & 0xff;
			ProtocolVersion pv = ProtocolVersion.valueOf(major, minor);

			CipherSuite cipherSuite = CipherSuite.valueOf(0, session.getCipher());
			SessionId sessionId = new SessionId(session.getSession_id());

			SignatureAndHashAlgorithm signAlg1 = SignatureAndHashAlgorithm.valueOf(4, 1, -1);// new
																								// SignatureAndHashAlgorithm(HashAlgorithm.SHA256,
																								// SignatureAlgorithm.RSA,
																								// "SHA256withRSA",
																								// 1);
			SignatureAndHashAlgorithm signAlg2 = SignatureAndHashAlgorithm.valueOf(2, 1, -1);// new
																								// SignatureAndHashAlgorithm(HashAlgorithm.SHA1,
																								// SignatureAlgorithm.RSA,
																								// "SHA1withRSA",
																								// 1);
			SignatureAndHashAlgorithm signAlg3 = SignatureAndHashAlgorithm.valueOf(5, 1, -1);// new
																								// SignatureAndHashAlgorithm(HashAlgorithm.SHA384,
																								// SignatureAlgorithm.RSA,
																								// "SHA384withRSA",
																								// 1);
			Collection<SignatureAndHashAlgorithm> algorithms = new ArrayList<>();
			algorithms.add(signAlg1);
			algorithms.add(signAlg2);
			algorithms.add(signAlg3);
			SSLSessionImpl sessionImpl = new SSLSessionImpl(pv, cipherSuite, algorithms, sessionId, hostname, port,
					false, null);

			SecretKeySpec sks = new SecretKeySpec(session.getMaster_key(), "TlsMasterSecret");

			sessionImpl.setMasterSecret(sks);

			// Session retrieved from PayPal session cache hence set
			// PPSavedSession flag.
			sessionImpl.setPPSavedSession(true);

			if (PayPalJSSEUtil.isResumptionLogOn()) {
				String message = "Reading Session for Session ID : " + sessionImpl.getSessionId();
				PayPalJSSEUtil.logMessage(message);
			}
