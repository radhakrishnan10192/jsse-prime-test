...
import com.paypal.infra.ssl.PayPalJSSEProperties;
...
final public class SSLSocketImpl extends BaseSSLSocketImpl {
    ...
    private static final Debug debug = Debug.getInstance("com/paypal/infra/ssl");
    ...
    SSLSocketImpl(SSLContextImpl context, boolean serverMode,
        ...
        super();
        // PayPal specific change to override mutual auth flag
        doClientAuth = PayPalJSSEProperties.getClientAuth();
    ...
    public void connect(SocketAddress endpoint, int timeout)
		...
		// PayPal specific change to disable dns reverse look up
		// JSSE was doing reverse lookup when host name not set explicitly.
		// reverse lookup causing some issue when
		// multiple record found for an IP
		String v = (String) AccessController
				.doPrivileged(new GetPropertyAction("com.paypal.jsse.dns.reverse.lookup.disable", "true"));
		if (v.compareTo("true") == 0) {
			InetSocketAddress epoint = (InetSocketAddress) endpoint;
			InetAddress addr = epoint.getAddress();
			String hostName = addr.getHostAddress();
			if (hostName.startsWith("10.")) // assume all internal IP starts
											// with "10."
				this.host = hostName;
		}
		// End
                ...
                try {
                    if (writeLock.tryLock(getSoLinger(), TimeUnit.SECONDS)) {
                        ...
                        if (isLayered() && !autoClose) {
                            ...
                            // to peer anymore.
                            fatal((byte)(-1), ssle);
                        } else if ((debug != null) && Debug.isOn("com/paypal/infra/ssl")) {
                            System.out.println(
      ...
      synchronized (readLock) {
        ...
        while (((state = getConnectionState()) != cs_CLOSED) &&
            ...
            try {
                ...
                if ((debug != null) && Debug.isOn("com/paypal/infra/ssl")) {
              ...
              switch (r.contentType()) {
                case Record.ct_handshake:
                    ...
                    if (handshaker.invalidated) {
						...
						// PayPal - Send handshake complete event for CAL
						// logging
						SSLSessionCacheProviderFactory.getSSLCacheProvider().handShakeCompleted(sess,
								!roleIsServer);
                    }
    ...
    private void sendAlert(byte level, byte description) {
        ...
        boolean useDebug = debug != null && Debug.isOn("com/paypal/infra/ssl");
        if (useDebug) {
            synchronized (System.out) {
                System.out.print(Thread.currentThread().getName());
    ...
    synchronized public void setNeedClientAuth(boolean flag) {
    	// PayPal specific change - commenting out below section to avoid client setting client auth option.
    	// Client auth will be configued via  protected package or keymaker.
        /*doClientAuth = (flag ?
    ...
    synchronized public void setEnabledCipherSuites(String[] suites) {
    	//PayPal specific change - to disable overriding cipher suite on SSLSocket
    	if(PayPalJSSEProperties.getClientCipherSuites() != null)	
        	return;
    	//end
    ...
    synchronized public void setEnabledProtocols(String[] protocols) {
    	// PayPal specific change - to disable client app setting protocol via SSLSocket.
    	// protocols are configured via protected package or keymaker.
    	if(PayPalJSSEProperties.getClientProtocolsEnabled() != null)
    		return;
    	// End
